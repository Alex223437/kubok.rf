<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
  ...
  <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
    <div class="pt-2 pb-3 space-y-1">
      <x-responsive-nav-link :href="route('dashboard')" :active="Route::is('dashboard')">Dashboard
      </x-responsive-nav-link>
      <x-responsive-nav-link :href="route('admin-pages')" :active="Route::is('admin-pages')">Страницы кубков
      </x-responsive-nav-link>
      ...
      <div class="mt-3 space-y-1">
        ...
        <form method="POST" action="{{ route('logout') }}">
          ...
          <x-responsive-nav-link :href="route('logout')"
                                 onclick="event.preventDefault(); this.closest('form').submit();">Выйти
          </x-responsive-nav-link>
        </form>
      </div>
    </div>
